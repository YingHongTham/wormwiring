/*
 * lookup table for getting WormAtlas(wa) link from cellname
 * e.g., cellname = 'ADAL', the "stem" is 'ADA',
 * which is what cellnameWALinkDict gives;
 * the function all the way at the bottom,
 * cellnameToWALink,
 * returns the WormAtlas link, in this case,
 * https://www.wormatlas.org/neurons/Individual%20Neurons/ADAmainframe.htm
 *
 * as pointed out in selectorCells.php,
 * this can probably be implemented with a prefix tree
 * but since the ratio of cellnames to walinks is low,
 * probably more efficient to just use a dictionary
 * (prefix tree would be fun though!)
 *
 * seems like these are only for neurons?
 */


const cellnameWALinkDict = {
  'ADAL': 'ADA',
  'ADAR': 'ADA',
  'ADEL': 'ADE',
  'ADER': 'ADE',
  'ADFL': 'ADF',
  'ADFR': 'ADF',
  'ADLL': 'ADL',
  'ADLR': 'ADL',
  'AFDL': 'AFD',
  'AFDR': 'AFD',
  'AIAL': 'AIA',
  'AIAR': 'AIA',
  'AIBL': 'AIB',
  'AIBR': 'AIB',
  'AIML': 'AIM',
  'AIMR': 'AIM',
  'AINL': 'AIN',
  'AINR': 'AIN',
  'AIYL': 'AIY',
  'AIYR': 'AIY',
  'AIZL': 'AIZ',
  'AIZR': 'AIZ',
  'ALA': 'ALA',
  'ALML': 'ALM',
  'ALMR': 'ALM',
  'ALNL': 'ALN',
  'ALNR': 'ALN',
  'AQR': 'AQR',
  'AS01': 'AS',
  'AS02': 'AS',
  'AS03': 'AS',
  'AS04': 'AS',
  'AS05': 'AS',
  'AS06': 'AS',
  'AS07': 'AS',
  'AS08': 'AS',
  'AS09': 'AS',
  'AS10': 'AS',
  'AS11': 'AS',
  'ASEL': 'ASE',
  'ASER': 'ASE',
  'ASGL': 'ASG',
  'ASGR': 'ASG',
  'ASHL': 'ASH',
  'ASHR': 'ASH',
  'ASIL': 'ASI',
  'ASIR': 'ASI',
  'ASJL': 'ASJ',
  'ASJR': 'ASJ',
  'ASKL': 'ASK',
  'ASKR': 'ASK',
  'AUAL': 'AUA',
  'AUAR': 'AUA',
  'AVAL': 'AVA',
  'AVAR': 'AVA',
  'AVBL': 'AVB',
  'AVBR': 'AVB',
  'AVDL': 'AVD',
  'AVDR': 'AVD',
  'AVEL': 'AVE',
  'AVER': 'AVE',
  'AVFL': 'AVF',
  'AVFR': 'AVF',
  'AVG': 'AVG',
  'AVHL': 'AVH',
  'AVHR': 'AVH',
  'AVJL': 'AVJ',
  'AVJR': 'AVJ',
  'AVKL': 'AVK',
  'AVKR': 'AVK',
  'AVL.': 'AVL',
  'AVL': 'AVL',
  'AVM': 'AVM',
  'AWAL': 'AWA',
  'AWAR': 'AWA',
  'AWBL': 'AWB',
  'AWBR': 'AWB',
  'AWCL': 'AWC',
  'AWCR': 'AWC',
  'BAGL': 'BAG',
  'BAGR': 'BAG',
  'BDUL': 'BDU',
  'BDUR': 'BDU',
  'CA01': 'CA',
  'CA02': 'CA',
  'CA03': 'CA',
  'CA04': 'CA',
  'CA05': 'CA',
  'CA06': 'CA',
  'CA07': 'CA',
  'CA08': 'CA',
  'CA09': 'CA',
  'CANL': 'CAN',
  'CANR': 'CAN',
  'CEMDL': 'CEM',
  'CEMDR': 'CEM',
  'CEMVL': 'CEM',
  'CEMVR': 'CEM',
  'CEPDL': 'CEP',
  'CEPDR': 'CEP',
  'CEPVL': 'CEP',
  'CEPVR': 'CEP',
  'CP0': 'CP',
  'CP01': 'CP',
  'CP02': 'CP',
  'CP03': 'CP',
  'CP04': 'CP',
  'CP05': 'CP',
  'CP06': 'CP',
  'CP07': 'CP',
  'CP08': 'CP',
  'CP09': 'CP',
  'DA01': 'DA',
  'DA02': 'DA',
  'DA03': 'DA',
  'DA04': 'DA',
  'DA05': 'DA',
  'DA06': 'DA',
  'DA07': 'DA',
  'DA08': 'DA',
  'DA09': 'DA',
  'DB01': 'DB',
  'DB02': 'DB',
  'DB03': 'DB',
  'DB04': 'DB',
  'DB05': 'DB',
  'DB06': 'DB',
  'DB07': 'DB',
  'DD01': 'DD',
  'DD02': 'DD',
  'DD03': 'DD',
  'DD04': 'DD',
  'DD05': 'DD',
  'DD06': 'DD',
  'DVA': 'DVA',
  'DVB': 'DVB',
  'DVC': 'DVC',
  'DVE': 'DVE',
  'DVF': 'DVF',
  'DX1': 'DX',
  'DX2': 'DX',
  'DX3': 'DX',
  'DX4': 'DX',
  'EF1': 'EF',
  'EF2': 'EF',
  'EF3': 'EF',
  'EF4': 'EF',
  'FLPL': 'FLP',
  'FLPR': 'FLP',
  'HOA': 'HOA',
  'HOB': 'HOB',
  'HSNL': 'HSN',
  'HSNR': 'HSN',
  'I1L': 'I1',
  'I1R': 'I1',
  'I2L': 'I2',
  'I2R': 'I2',
  'I3': 'I3',
  'I4': 'I4',
  'I5': 'I5',
  'I6': 'I6',
  'IL1DL': 'IL1',
  'IL1DR': 'IL1',
  'IL1L': 'IL1',
  'IL1R': 'IL1',
  'IL1VL': 'IL1',
  'IL1VR': 'IL1',
  'IL2DL': 'IL2',
  'IL2DR': 'IL2',
  'IL2L': 'IL2',
  'IL2R': 'IL2',
  'IL2VL': 'IL2',
  'IL2VR': 'IL2',
  'LUAL': 'LUA',
  'LUAR': 'LUA',
  'M1': 'M1',
  'M2L': 'M2',
  'M2R': 'M2',
  'M3L': 'M3',
  'M3R': 'M3',
  'M4': 'M4',
  'M5': 'M5',
  'MCL': 'MC',
  'MCR': 'MC',
  'MCML': 'MCM',
  'MCMR': 'MCM',
  'MI': 'MI',
  'NSML': 'NSM',
  'NSMR': 'NSM',
  'OLLL': 'OLL',
  'OLLR': 'OLL',
  'OLQDL': 'OLQ',
  'OLQDR': 'OLQ',
  'OLQVL': 'OLQ',
  'OLQVR': 'OLQ',
  'PCAL': 'PCA',
  'PCAR': 'PCA',
  'PCBL': 'PCB',
  'PCBR': 'PCB',
  'PCCL': 'PCC',
  'PCCR': 'PCC',
  'PDA': 'PDA',
  'PDB': 'PDB',
  'PDC': 'PDC',
  'PDEL': 'PDE',
  'PDER': 'PDE',
  'PGA': 'PGA',
  'PHAL': 'PHA',
  'PHAR.': 'PHA',
  'PHAR': 'PHA',
  'PHBL': 'PHB',
  'PHBR': 'PHB',
  'PHCL': 'PHC',
  'PHCR': 'PHC',
  'PLML': 'PLM',
  'PLMR': 'PLM',
  'PLNL': 'PLN',
  'PLNR': 'PLN',
  'PQR': 'PQR',
  'PVCL': 'PVC',
  'PVCR': 'PVC',
  'PVDL': 'PVD',
  'PVDR': 'PVD',
  'PVM': 'PVM',
  'PVNL': 'PVN',
  'PVNR': 'PVN',
  'PVPL': 'PVP',
  'PVPR': 'PVP',
  'PVQL': 'PVQ',
  'PVQR': 'PVQ',
  'PVR.': 'PVR',
  'PVR': 'PVR',
  'PVS': 'PVS',
  'PVT': 'PVT',
  'PVU': 'PVU',
  'PVV': 'PVV',
  'PVWL': 'PVW',
  'PVWR': 'PVW',
  'PVY': 'PVY',
  'PVX': 'PVX',
  'PVZ': 'PVZ',
  'R1AL': 'R1A',
  'R1AR': 'R1A',
  'R1BL': 'R1B',
  'R1BR': 'R1B',
  'R2AL': 'R2A',
  'R2AR': 'R2A',
  'R2BL': 'R2B',
  'R2BR': 'R2B',
  'R3AL': 'R3A',
  'R3AR': 'R3A',
  'R3BL': 'R3B',
  'R3BR': 'R3B',
  'R4AL': 'R4A',
  'R4AR': 'R4A',
  'R4BL': 'R4B',
  'R4BR': 'R4B',
  'R5AL': 'R5A',
  'R5AR': 'R5A',
  'R5BL': 'R5B',
  'R5BR': 'R5B',
  'R6AL': 'R6A',
  'R6AR': 'R6A',
  'R6BL': 'R6B',
  'R6BR': 'R6B',
  'R7AL': 'R7A',
  'R7AR': 'R7A',
  'R7BL': 'R7B',
  'R7BR': 'R7B',
  'R8AL': 'R8A',
  'R8AR': 'R8A',
  'R8BL': 'R8B',
  'R8BR': 'R8B',
  'R9AL': 'R9A',
  'R9AR': 'R9A',
  'R9BL': 'R9B',
  'R9BR': 'R9B',
  'RIAL': 'RIA',
  'RIAR': 'RIA',
  'RIBL': 'RIB',
  'RIBR': 'RIB',
  'RICL': 'RIC',
  'RICR': 'RIC',
  'RID': 'RID',
  'RIFL': 'RIF',
  'RIFR': 'RIF',
  'RIGL': 'RIG',
  'RIGR': 'RIG',
  'RIH': 'RIH',
  'RIML': 'RIM',
  'RIMR': 'RIM',
  'RIPL': 'RIP',
  'RIPR': 'RIP',
  'RIR': 'RIR',
  'RIS': 'RIS',
  'RIVL': 'RIV',
  'RIVR': 'RIV',
  'RMDDL': 'RMD',
  'RMDDR': 'RMD',
  'RMDL': 'RMD',
  'RMDR': 'RMD',
  'RMDVL': 'RMD',
  'RMDVR': 'RMD',
  'RMED': 'RME',
  'RMEL': 'RME',
  'RMER': 'RME',
  'RMEV': 'RME',
  'RMFL': 'RMF',
  'RMFR': 'RMF',
  'RMGL': 'RMG',
  'RMGR': 'RMG',
  'RMHL': 'RMH',
  'RMHR': 'RMH',
  'SAADL': 'SAA',
  'SAADR': 'SAA',
  'SAAVL': 'SAA',
  'SAAVR': 'SAA',
  'SABD': 'SAB',
  'SABVL': 'SAB',
  'SABVR': 'SAB',
  'SDQL': 'SDQ',
  'SDQR': 'SDQ',
  'SIADL': 'SIA',
  'SIADR': 'SIA',
  'SIAVL': 'SIA',
  'SIAVR': 'SIA',
  'SIBDL': 'SIB',
  'SIBDR': 'SIB',
  'SIBVL': 'SIB',
  'SIBVR': 'SIB',
  'SMBDL': 'SMB',
  'SMBDR': 'SMB',
  'SMBVL': 'SMB',
  'SMBVR': 'SMB',
  'SMDDL': 'SMD',
  'SMDDR': 'SMD',
  'SMDVL': 'SMD',
  'SMDVR': 'SMD',
  'SPCL': 'SPC',
  'SPCR': 'SPC',
  'SPDL': 'SPD',
  'SPDR': 'SPD',
  'SPVL': 'SPV',
  'SPVR': 'SPV',
  'URADL': 'URA',
  'URADR': 'URA',
  'URAVL': 'URA',
  'URAVR': 'URA',
  'URBL': 'URB',
  'URBR': 'URB',
  'URXL': 'URX',
  'URXR': 'URX',
  'URYDL': 'URY',
  'URYDR': 'URY',
  'URYVL': 'URY',
  'URYVR': 'URY',
  'VA01': 'VA',
  'VA02': 'VA',
  'VA03': 'VA',
  'VA04': 'VA',
  'VA05': 'VA',
  'VA06': 'VA',
  'VA07': 'VA',
  'VA08': 'VA',
  'VA09': 'VA',
  'VA10': 'VA',
  'VA11': 'VA',
  'VA12': 'VA',
  'VB01': 'VB',
  'VB02': 'VB',
  'VB03': 'VB',
  'VB04': 'VB',
  'VB05': 'VB',
  'VB06': 'VB',
  'VB07': 'VB',
  'VB08': 'VB',
  'VB09': 'VB',
  'VB10': 'VB',
  'VB11': 'VB',
  'VC01': 'VC',
  'VC02': 'VC',
  'VC03': 'VC',
  'VC04': 'VC',
  'VC05': 'VC',
  'VC06': 'VC',
  'VD01': 'VD',
  'VD02': 'VD',
  'VD03': 'VD',
  'VD04': 'VD',
  'VD05': 'VD',
  'VD06': 'VD',
  'VD07': 'VD',
  'VD08': 'VD',
  'VD09': 'VD',
  'VD10': 'VD',
  'VD11': 'VD',
  'VD12': 'VD',
  'VD13': 'VD',
}

function cellnameToWALink(cellname) {
  let stem = cellnameWALinkDict[cellname];
  return `https://www.wormatlas.org/neurons/Individual%20Neurons/${stem}mainframe.htm`;
}
